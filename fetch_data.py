...
for pair in pairs:
    ...
    while flag:
        ...
        print('Current Date: ', data.datetime.iloc[-1])
        print("End Date: ", end_date_ts - datetime.timedelta(minutes=15))

    data.to_csv(f"data/{str(exchange).lower()}_{pair.replace('/', '-').lower()}_{timeframe}_candles_{start_date}.csv", index=False)
